...
async function loadGifts(){
  const wrap = document.querySelector('#giftList');
  if(!wrap) return;
  try{
    const res = await fetch('data/gifts.json', {cache:'no-store'});
    const gifts = await res.json();
    wrap.innerHTML = gifts.map(gift => `
      <article class="gift-card">
        <span>${gift.tag}</span>
        <b>${gift.name}</b>
        <ul>${gift.items.map(item => `<li>${item}</li>`).join('')}</ul>
        <p>${gift.note}</p>
      </article>
    `).join('');
  }catch(e){ console.warn('gifts json load failed', e); }
}
loadGifts();
